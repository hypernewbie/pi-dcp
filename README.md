...
- **Targeted history retrieval**: the agent can read a small raw excerpt from earlier active-session history when a specific original request, error, or tool result is missing from current context. It is read-only and bounded; it cannot dump a full session. The transcript shows a compact result by default; expand tool output to inspect the complete excerpt the model received.
